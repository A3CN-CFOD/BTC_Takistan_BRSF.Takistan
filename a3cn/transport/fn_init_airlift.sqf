...
a3cn_airlift = ["typeofhelicopter","spawn pos drop point",50 alt, 30 min delay] spawn a3cn_fnc_init_airlift;
*/
...
_timedelay = (player getVariable "a3cn_airlift");
if (Time < _timedelay) exitwith {hint format ["Airlift Helicopter will be available in %1",((_timedelay - Time) call a3cn_fnc_timer)];};
...
openMap false;
	...
	if ((isnil "a3cn_vehsel") or ! DLG_USE_SELECTED) exitwith {hint "Nothing Selected";};
	
player setVariable ["a3cn_airlift", Time + _delay];

[_liftObject,_spawnmark,_airtype,_flyheight,player] remoteExec ["a3cn_fnc_airliftserver",2];
