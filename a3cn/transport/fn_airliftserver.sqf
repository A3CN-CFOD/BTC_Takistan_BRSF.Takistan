...
{_air1 lockTurret [_x, true];} foreach allTurrets _air1;
	...
	if !(_air1 canSlingLoad _liftObject) exitwith {
		...
		_p setVariable ["a3cn_airlift", 0, true];
	};

_airliftgrp setBehaviour "CARELESS";

_wp = [_airliftgrp,_pos, 5, "HOOK", "CARELESS", "NORMAL", "WEDGE"] call a3cn_fnc_createwaypoint;
...
_liftpad_mark = [_pos,"ColorGreen","Airlift Sling Load","hd_pickup"] call a3cn_fnc_mark_point;
...
while { (alive _air1) and {canmove _air1} and {alive (driver _air1)} } do {
	...
	[_liftObject, "ColorGrey", "mil_box", _liftobj_name, true, true] spawn a3cn_fnc_tracker;
	_msg = format ["Sling loading %1", _liftobj_name];
	[_air1, _msg] remoteExec ["sideChat"];
	
	[_airliftgrp,_spawnmark, 5, "UNHOOK", "CARELESS", "NORMAL", "WEDGE"] call a3cn_fnc_createwaypoint;
	_air1 flyInHeight _flyheight;
