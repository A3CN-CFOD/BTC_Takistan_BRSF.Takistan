...
_side = [_this, 7, WEST,[WEST]] call BIS_fnc_param;//check to see if type is specified otherwise use default unitlist aircraft
_ghst_patrol_air_list = [_this, 8, ghst_attackplane,[[]]] call BIS_fnc_param;//check to see if type array is specified otherwise use default unitlist type array
...
for "_x" from 0 to (_airqty) - 1 do {
	...
	_air1_array = [_spawnpos, _dir, _air_type, _grp] call BIS_fnc_spawnVehicle;
	...
	//_crew = [_air1, _grp] call BIS_fnc_SpawnCrew;
	//sleep 1;
...
[_grp,_centermarker,_radarray,_markunitsarray,["AWARE", "NORMAL", "WEDGE"]] call ghst_fnc_waypoints;
...
[_spawnmarker,_centermarker,_radarray,_flyheight,_airqty,_rspawn,_markunitsarray,_side,_ghst_patrol_air_list] call ghst_fnc_bair;
