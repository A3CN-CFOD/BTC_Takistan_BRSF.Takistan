...
While {true} do {
	...
	_randnum = (random 10);
	sleep 10;
	if ((_ghst_side countSide allunits < 35) and {(_randnum > 6)} and {(count _curtasks > 0)} and {(Time > _timedelay)}) then {
	...
	if ((_ghst_side countSide allunits < 35) and {(_randnum < 4)} and {(count _curtasks > 0)} and {(_max_reinf < _numobjs)} and {(Time > _timedelay)}) then {
